...
function [x_arrays, quantiles] = compute_and_plot_p(x_arrays, arrs, ...
...
for i = 1:num_methods
    ...
    h = fill([x_arrays{i}'  fliplr(x_arrays{i}')], [maxs{i}' fliplr(mins{i}')], minmax_colors{i},'EdgeColor', 'none');
    set(h,'facealpha', .5)

    h = fill([x_arrays{i}'  fliplr(x_arrays{i}')], [quant75s{i}'  fliplr(quant25s{i}')], quant_colors{i},'EdgeColor', 'none');
    set(h,'facealpha', .5)

    if i==1
        plot_array(i) = plot( x_arrays{i}, medians{i}, line_colors{i}, 'LineWidth', 1,...
            ...
            plot_array(i) = plot( x_arrays{i}, medians{i}, line_colors{i}, 'LineWidth', 1,...
                'LineStyle', '-','Marker', 'd', 'DisplayName', display_names{i});
...
if display_legend
    if plotaxes
        legend(plot_array, display_names,'location', 'northwest');
    else
...
if isfield(opts,'rate')
    ...
    legend(plot_array, display_names,'location', 'northwest');
    txt=title( ['{\tt randn}',',$m=$ ',num2str(opts.m),',$n=$ ',num2str(opts.n),',$r=$ ',num2str(opts.rank),',$\kappa=$',num2str(opts.kappa)]);
    set(txt, 'Interpreter', 'latex');
else if isfield(opts,'comparison')
        xlabel('$\log_2(p)$', 'Interpreter', 'latex')
        ylabel('$k\cdot\frac{p}{m}$', 'Interpreter', 'latex')
        legend(plot_array, display_names,'location', 'northeast');
        txt=title( ['{\tt randn}',',$m=$ ',num2str(opts.m),',$n=$ ',num2str(opts.n),',$r=$ ',num2str(opts.rank),',$\kappa=$',num2str(opts.kappa)]);
        set(txt, 'Interpreter', 'latex');
else if isfield(opts,'CPU')
        xlabel('$\log_2(p)$', 'Interpreter', 'latex')
        ylabel('CPU time')
        legend(plot_array, display_names,'location', 'northeast');
        txt=title( ['{\tt randn}',',$m=$ ',num2str(opts.m),',$n=$ ',num2str(opts.n),',$r=$ ',num2str(opts.rank),',$\kappa=$',num2str(opts.kappa)]);
...
if plotaxes
    ...
    if isfield(opts,'axesposition')
        axesposition=opts.axesposition;
    else
    ...
    %if isfield(opts,'rate')
        xlim([opts.startp,opts.startp+3]);
    %else
       % xlim([opts.startp,opts.startp+4]);
    %end
end
